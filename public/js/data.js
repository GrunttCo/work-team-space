...
/* Hash simple (no criptográfico, suficiente para uso interno) */
...
function saveTasks(tasks, counter) {
  const payload = { tasks, counter: counter || _taskIdCounter };
  localStorage.setItem(TASKS_STORE, JSON.stringify(payload));
  pushToServer('tasks', payload);
